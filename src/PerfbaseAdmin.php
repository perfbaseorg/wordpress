...
class PerfbaseAdmin {
    ...
    public function register_privacy_policy_content() {
        ...
        $content = sprintf(
            ...
            __(
                'Perfbase is an external application performance monitoring service. When a Perfbase API key is configured and profiling is enabled, this site may send profiling traces to the configured Perfbase API endpoint, currently %s. Perfbase can send function call trees, function names, source file paths and line numbers, timing, memory, CPU and host resource metrics, runtime error or exception context, inbound WordPress request URLs without query strings, user IP address, user agent, logged-in user ID when available, hostname, environment, application version, PHP version, HTTP method, HTTP status code, and WordPress request context metadata. Depending on enabled extension features, loaded PHP libraries, and which code paths run during the trace, traces may also include normalized SQL query text, database connection metadata, MongoDB or Elasticsearch query/filter summaries, Redis or Memcached keys and fields, HTTP URL or URI metadata that may include query strings, file paths, mail recipient and subject metadata, shell/process command strings, AWS operation names, OPcache and JIT statistics, PHP error or exception samples, compiled file paths, magic method counts, process-list snapshots, and truncated function argument values if argument capture is separately configured. Perfbase does not collect source code, request bodies, full POST payloads or arbitrary form fields, uploaded file contents, cookie values, PHP session data, authorization header values, passwords, API keys, nonces, or session IDs from WordPress request, cookie, or session data, or command-line arguments for process-list snapshots. Administrators should review enabled feature flags before profiling sensitive workloads. Perfbase does not submit profiling traces when the API key is missing or profiling is disabled.',
                'perfbase'
            ),
